...
class AnalyzeAgent:
    ...
    def opposite_opinion_association(self, full_topic, stream=True, on_chunk=None, print_stream=True):
        ...
        response = self.llm.chat(messages, temperature=0.5, stream=stream, on_chunk=on_chunk, print_stream=print_stream)
            
        self.save_to_file(response, "opposite_opinion.txt")
        return response

    def analyze_critique(self, stream=True, on_chunk=None, print_stream=True):
        ...
        critique = self.llm.chat(messages, temperature=0.6, stream=stream, on_chunk=on_chunk, print_stream=print_stream)
        # 保存反方批判
        self.save_to_file(critique, "critique.txt")
        
        return critique

    def analyze_significance(self, full_topic, stream=True, on_chunk=None, print_stream=True):
        ...
        significance = self.llm.chat(messages, temperature=0.6, stream=stream, on_chunk=on_chunk, print_stream=print_stream)
        # 保存正方现实意义
