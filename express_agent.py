...
class ExpressAgent:
    ...
    def build_self_expression(self, full_topic, stream=True, on_chunk=None, print_stream=True):
    ...
    def build_golden_sentence(self, full_topic, stream=True, on_chunk=None, print_stream=True):
        opposite_opinion = self.load_from_file("opposite_opinion.txt", self.STAGE1_DIR)
        ...
        response = self.llm.chat(messages, temperature=0.6, stream=stream, on_chunk=on_chunk, print_stream=print_stream)
        if not response:
            response = ""
        response = '\n'.join(line.strip() for line in response.splitlines() if line.strip())
